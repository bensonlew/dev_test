...
class ManagerProcess(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    if wsheet.batch and wsheet.instant is not True:
                        ...
                        self.process_start(json_data["id"], worker.pid)
                except Exception, e:
                    exstr = traceback.format_exc()
                    print exstr
                    print e
            ...
            if len(self.process) <= max_process:
                if len(self._batch_queue) > 0:
                    ...
                    try:
                        ...
                        self.process_start(sheet.id, worker.pid)
                    except Exception, e:
                        exstr = traceback.format_exc()
                        print exstr
                        print e
                ...
                if added:
                    self.logger.info("正在运行的任务数为%s,当前排队一键化任务数为%s" %
                                     (len(self.process), len(self._batch_queue)))
    ...
    def process_start(self, wid, pid):
        ...
        add_run_queue(self.run_info_queue, "start", wid, pid)
