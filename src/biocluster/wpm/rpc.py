                    ...
                    with self.message_queue.get_lock():
                        for i, v in enumerate(self.message_queue):
                            if v.id == "":
                                if len(data_list) > 0:
                                    msg = data_list.pop(0)
                                    self.message_queue[i] = (msg["id"], msg["state"], msg["data"], msg["version"],
                                                             int(time.time()))
                                ...
                                if len(data_list) > 0:
                                    ...
                                    for m in data_list:
                                        if m["id"] == v.id and v.state == "keepalive":
                                            found = m
                                            self.message_queue[i] = (m["id"], m["state"], m["data"], m["version"],
                                                                     int(time.time()))
...
class RPC(object):
    ...
    def _start_listen(self):
        try:
            ...
            if self._retry_times < 3:
                ...
                self.manager.logger.exit("启动监听错误超过3次，终止运行!")
                return
    ...
    def restart(self):
        if self._last_start and (int(time.time()) - self._last_start) < 300:
            ...
            return
        self._retry_times = 0
        self.manager.logger.info("重启监听服务%s" % self.manager.endpoint)
        self._rpc_server.close()
        gevent.sleep(3)
        gevent.spawn(self._start_listen)
