        ...
        while True:
            try:
                ...
                for log in log_queue:
                    if log.webapi_end is True and log.update_status_end is True:
                        end_list.append(log)
                        self.manager.log_object_queue.pop(id(log))
                    elif log.has_update_webapi is True and log.update_status_success is False:
...
class Log(object):
    ...
    def send(self):
        ...
        post_data = "%s&%s" % (self.get_sig(), self.post_data)
    ...
    def _update_web_api(self):
        ...
        try:
            ...
            if self.failed_times < self.config.UPDATE_MAX_RETRY:
                self.logger.error(" 更新数据库状态出错:%s,重试..." % e)
            else:
                self.webapi_end = True
                self.logger.error(" 更新数据库状态出错:%s,超过最大错误次数，终止尝试...." % e)
        else:
