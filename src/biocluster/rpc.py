...
class RPC(object):
    ...
    def _get_rpc_info(self):
        ...
        while True:
            ...
            if self.last_none_block:
                ...
                if block_time >= 600:
                    self._workflow.logger.info("进程被阻塞超过%s秒，可能已经导致RPC数据丢失!请减少阻塞时间！"
                                               "多个阻塞循环可在循环中使用gevent.sleep(0)跳出阻塞!" % block_time)
                elif block_time > 30:
                    self._workflow.logger.info("进程被阻塞超过%s秒，长时间阻塞会导致RPC数据丢失,请减少阻塞时间! "
                                               "多个阻塞循环可在循环中使用gevent.sleep(0)跳出阻塞!" % block_time)
            try:
    ...
    def _start_listen(self):
        try:
            ...
            if self._retry_times < 3:
                gevent.sleep(3)
                self._workflow.logger.debug("启动监听错误,尝试重试1")
                self._start_listen()
            else:
                self._workflow.exit("启动监听错误超过3次，终止运行!")
                return
    ...
    def restart(self):
        ...
        if self._workflow.sheet.WPM:
            ...
            self._retry_times = 0
            self._workflow.logger.info("重启监听服务%s" % self.endpoint)
            self._rpc_server.close()
            gevent.sleep(3)
            gevent.spawn(self._start_listen)
