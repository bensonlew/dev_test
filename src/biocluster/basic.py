...
class StepMain(Step):
    ...
    def upload_files(self):
        ...
        if workflow.sheet.interaction and self.bind_obj.sheet.output:
            index = self.bind_obj.sheet.output.find("report_results")
            if index > 0:
                dir_list.append({"path": self.bind_obj.sheet.output[0:index] + "report_results", "size": "",
                                 "description": "交互分析结果目录", "format": ""})
    ...
    def update(self):
        ...
        if self.has_change:
            ...
            workflow_log = {
                ...
                "desc": filter_error_info(self._info)
            }
        ...
        for step in self._steps.values():
            if step.has_change:
                state = {
                    ...
                    "time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "desc": filter_error_info(step._info)
                }
