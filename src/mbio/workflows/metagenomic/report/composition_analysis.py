...
class CompositionAnalysisWorkflow(Workflow):
    def __init__(self, wsheet_object):
        self._sheet = wsheet_object
        super(CompositionAnalysisWorkflow, self).__init__(wsheet_object)
        options = [
            ...
            {"name": "specimen_group", "type": "string"},
            {"name": "analysis", "type": "string", "default": "bar"},  # bar,heatmap,circos
    ...
    def set_db(self):
        ...
        if self.option("analysis") in ["heatmap"]:
            specimen_tree = ""
            species_tree = ""
            if self.option("method") != "":
                species_tree = self.composition_analysis.output_dir.rstrip('/') + '/heatmap/specimen_hcluster.tre'
            if self.option("sample_method") != "":
                specimen_tree = self.composition_analysis.output_dir.rstrip('/') + '/heatmap/species_hcluster.tre'
        ...
        elif self.option("analysis") in ["bar"]:
            ...
            api_composition.add_composition_detail(file_path, self.option("main_id"), species_tree="",
                                                   specimen_tree="")
        elif self.option("analysis") in ["circos"]:
    ...
    def end(self): 
        ...
        super(CompositionAnalysisWorkflow, self).end()
    ...
    def run(self):
        ...
        super(CompositionAnalysisWorkflow, self).run()
