...
class PtBatchWorkflow(Workflow):
	def __init__(self, wsheet_object):
		'''
		:param wsheet_object:
		'''
		self._sheet = wsheet_object
		super(PtBatchWorkflow, self).__init__(wsheet_object)
		options = [
	...
	def fastq2mongo_run(self):
		...
		for i in file:
			if not api_read_tab.tab_exist(i):
				fastq2mongo = self.add_module("paternity_test.fastq2mongo")
	...
	def pt_analysis_run(self):
		...
		dad = self.family_id[0]
		mom = self.family_id[1]
		preg = self.family_id[2]
		n=0
		for id in dad:
			dad_id = id
			for m_id in mom:
				...
				for p_id in preg:
					...
					pt_analysis.set_options({
							"dad_tab": api_read_tab.export_tab_file(dad_id, self.output_dir),  # 数据库的tab文件
	...
	def result_info_run(self,event):
		...
		result_info.on('end', self.dedup_run)
		result_info.run()
	...
	def run(self):
		...
		super(PtBatchWorkflow, self).run()
	...
	def end(self):
		...
		for dad_id in dad:
			for mom_id in mom:
				for preg_id in preg:
					self.father_id=api_main.add_sg_father(dad_id, mom_id, preg_id)
					...
					for f in results:
						if f == dad_id + '_' + mom_id +'_' + preg_id + '_family_analysis.txt':
		...
		super(PtBatchWorkflow,self).end()
