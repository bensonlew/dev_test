...
class PtBatchWorkflow(Workflow):
	...
	def dedup_run(self,event):
		...
		p = event['data']
		print p
		print self.family_id[p][0]
		print type(self.family_id[p][0])
		...
		for i in name_list:
			...
			pt_analysis_dedup.set_options({
					...
					"preg_tab": api_read_tab.export_tab_file(self.family_id[p][2], self.output_dir),
					"ref_point": self.option("ref_point").prop['path'],
					"err_min": self.option("err_min")
			}
			)
	...
	def end(self):
		...
		for i in range(len(self.family_id)):
			...
			#更新总的状态表
			api_update_status.update_flow_status(self.option('batch_id'))
