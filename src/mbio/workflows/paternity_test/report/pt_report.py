...
# last modified by hongdongxuan :20170918
...
class PtReportWorkflow(Workflow):
    ...
    def dedup_run(self):
        ...
        if self.option('dedup_num') != "all":
            ...
            if num <= int(self.option('dedup_num')):
                raise Exception("查重的范围太大，请调小查重范围!")
            num_list = range(num - int(self.option('dedup_num')) - 1, num + int(self.option('dedup_num')) + 1)
            name_list = []
            for m in num_list:
            ...
            self.logger.info("进行查重的父本:%s" % name_list)
            for m in name_list:
                api_read_tab.export_tab_file(str(m), father_data)
            self.logger.info("导出查重父本样本tab完成！")
