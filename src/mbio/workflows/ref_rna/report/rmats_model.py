...
class RmatsModelWorkflow(Workflow):
    ...
    def __init__(self, wsheet_object):
        ...
        options = [
            ...
            {"name": "rmats_out_root_dir", "type": "string"},
    ...
    def run_rmats_model(self):
        a_bam_str = ','.join([line.strip() for line in subprocess.check_output(
            os.path.join('ls {}'.format(os.path.join(self.option('rmats_out_root_dir'), 'SAMPLE_1/*/*.bam'))),
            shell=True).strip().split(
            '\n')])
        b_bam_str = ','.join([line.strip() for line in subprocess.check_output(
            os.path.join('ls {}'.format(os.path.join(self.option('rmats_out_root_dir'), 'SAMPLE_2/*/*.bam'))),
            shell=True).strip().split(
            '\n')])
        rmats_event_file = os.path.join(self.option("rmats_out_root_dir"),
    ...
    def set_db(self):
        ...
        api_rmats_model.add_sg_fs(result_files[0], ObjectId(rmats_model_id))
