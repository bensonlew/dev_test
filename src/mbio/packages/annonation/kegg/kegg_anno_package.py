...
class kegg_annotation(object):
    def __init__(self):
        ...
        self.output_path = os.path.join(os.path.join(os.getcwd(), 'output'))
    def pathSearch(self, blast_xml):#输入blast比对的xml文件
        """
        输入blast比对的xml文件(Trinity_vs_kegg.xml)，输出kegg_table.xls
        """
        tablefile = open(os.path.join(self.output_path, 'kegg_table.xls'), "wb")
    ...
    def pathTable(self):
        ...
        """
        path_table_xls = open(os.path.join(self.output_path, "path_table.xls"), "wb")#输出文件path_table.xls
        pid_txt = open("pid.txt", "wb")#输出文件pid.txt
        ...
        kegg_table = islice(open(os.path.join(self.output_path, "kegg_table.xls")), 1, None)#打开kegg_table.xls
    ...
    def getPic(self):
        ...
        f = open("pid.txt")
        if not os.path.exists(os.path.join(self.output_path, 'pathways')):
            os.makedirs(os.path.join(self.output_path, 'pathways'))
        pathways_path = os.path.join(self.output_path, 'pathways')
        print pathways_path
        for i in f:
            if i:
                i = i.strip('\n').split('\t')
                ...
                for ko in koid:
                    ...
                    canvas.draw(pathways_path + '/' + pid + '.pdf')
    ...
    def keggLayer(self):
        ...
        f = open(os.path.join(self.output_path, "path_table.xls"))
        d = {}
        ...
        with open(os.path.join(self.output_path, "kegg_layer.xls"), "w+") as k, open(os.path.join(self.output_path, "kegg_taxonomy.xls"), "w+") as t:
            for i in d:
                n = 0
                doc = ''
                for j in d[i]:
