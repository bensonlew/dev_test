            ...
            if internal_txpt_id_m and cls_m and internal_gene_id_m:
                ...
                raise Exception(
                    'line: {} in annotate gtf {} has no valid internal txpt id or class code'.format(line.strip(),
                                                                                                     cmp_file))
            d[internal_txpt_id] = {'ref_txpt_id': ref_txpt_id, 'cls': cls, 'internal_gene_id': internal_gene_id}
    return d
...
def write_text(ref_dic, cmp_dic, f):
    fw = open(f, 'w')
    head = '#{}\n'.format('\t'.join(['assemble_txpt_id', 'assemble_gene_id', 'class_code', 'ref_txpt_id', 'ref_gene_id', 'ref_gene_name']))
    fw.write(head)
    ...
    for internal_id in cmp_dic.keys():
        ...
        newline = '\t'.join([internal_id, internal_gene_id, cls, ref_tid, ref_gid, ref_gname]) + '\n'
        fw.write(newline)
