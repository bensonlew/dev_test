...
def stat_info(trinity_info, gene_path, stat_path, len_dir_path, length):
    ...
    with open(gene_path, 'wb') as g, open(stat_path, 'wb') as s:
        ...
        s.write('\tunigenes\ttranscripts\ntotal seq num\t%s\t%s\ntotal base num\t%s\t%s\npercent GC\t%s\t%s\nlargest transcript\t%s\t%s\nsmallest transcript\t%s\t%s\naverage length\t%s\t%s\nN50\t%s\t%s\nN90\t%s\t%s' % (gene_seq_num, tran_seq_num, gene_base_num, tran_base_num, '%0.4g' % gene_GC_per, '%0.4g' % tran_GC_per, gene_max_len, tran_max_len, gene_min_len, tran_min_len, '%0.4g' % gene_average_len, '%0.4g' % tran_average_len, gene_N50, tran_N50, gene_N90, tran_N90))
        for one in gene_len_distri:
            ...
            with open("{}/{}_length.distribut.txt".format(len_dir_path, one), "wb") as l:
                ...
                start = 1
                for i in range(one, gene_max_len + one, one):
...
def len_stat(len_list, base_num):
    ...
    for i in len_list:
        if len50 < base_num * 0.5:
            ...
            len90 += i
            N90 = i
    return max_len, min_len, average_len, N50, N90
