...
class SgPaternityTest(Base):
    ...
    def add_father_result(self, father_id, pt_father_id):
        ...
        dad_id = case['dad_id']
        result_case = collection_result.find_one(
            {'pt_father_id': pt_father_id, "dad_id": dad_id})
        result = result_case['result']

        try:
            collection.update({"_id": father_id}, {
                              '$set': {"pt_father_id": pt_father_id, 'result': result}})
        except Exception as e:
    ...
    def update_sg_pt_father(self, pt_father_id):
        try:
            ...
            collection.update({"_id": pt_father_id}, {
                              '$set': {"status": "end"}})
        except Exception as e:
