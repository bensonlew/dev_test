...
class HcHeatmap(Base):
    ...
    def heatmap_in(self):
        ...
        with open(self.output_dir + '/result_data') as f:
            self.bind_object.logger.info("hc_heatmap主表写入")
            hc_heatmap_id = self.db['hc_heatmap'].insert_one(SON(
            ...
            self.bind_object.logger.info("hc_heatmap主表写入结束")
            insert_data = []
            head = f.next().strip('\r\n')  # windows换行符
            head = re.split('\t', head)
            new_head = head[1:]
            for line in f:
                ...
                sample_num = line[1:]
                otu_detail = dict()
                otu_detail['hc_heatmap_id'] = hc_heatmap_id
                otu_detail['row_name'] = line[0]
                for i in range(0, len(sample_num)):
                    otu_detail[new_head[i]] = sample_num[i]
                insert_data.append(otu_detail)
            self.bind_object.logger.info("hc_heatmap表格数据导入开始")
            try:
                self.db['hc_heatmap_detail'].insert_many(insert_data)
            except Exception as e:
                self.bind_object.logger.info("hc_heatmap数据表导入出错{}".format(e))
            else:
                self.bind_object.logger.info("hc_heatmap数据表导入完成")
            ...
            if os.path.exists(col_tree_path):
                ...
                col_tree = ""
                col_list = []
            self.bind_object.logger.info("hc_heatmap_id：{}".format(hc_heatmap_id))
            try:
                self.db['hc_heatmap'].update_one({'_id': hc_heatmap_id}, {'$set':
                                                            {'status': 'end',
                                                            'row_tree': row_tree,
                                                            'row_list': row_list,
                                                            'col_tree': col_tree,
                ...
                self.bind_object.logger.info("hc_heatmap主表更新出错{}".format(e))
            else:
                self.bind_object.logger.info("hc_heatmap主表更新完成")
            return hc_heatmap_id
