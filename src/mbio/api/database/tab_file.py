...
class TabFile(Base):
        ...
        with open(sample, 'r') as f:
            ...
            try:
                ...
                collection.find_one_and_update(
                    {"sample_id": sample_name}, {'$set': insert_data})
    ...
    def update_pt_tab(self, sample):
        try:
            collection = self.database['sg_pt_ref_main']
            collection.update({"sample_id": sample}, {
                              '$set': {"analysised": "yes"}})
        except Exception as e:
    ...
    def tab_exist(self, sample):
        ...
        if not result:
            self.bind_object.logger.info(
                "样本{}不在数据库中，开始进行转tab文件并入库流程".format(sample))
        else:
            self.bind_object.logger.info(
                "样本{}已存在数据库中，不进行fastq转tab流程".format(sample))
        return result
    ...
    def export_tab_file(self, sample, dir):
        ...
        if temp:
            ...
            if collection.find_one({"sample_id": sample_other_name}):
                final_result = collection.find(
                    {"sample_id": sample_other_name})
    ...
    def sample_qc_addition(self, sample_id):
        ...
        find_n_hit = collection.find_one(
            {"sample_id": sample_id, 'qc': 'n_hit'})
        ...
        find_n_hit_dedup = collection.find_one(
            {"sample_id": sample_id, 'qc': 'n_hit_dedup'})
    ...
    def family_unanalysised(self):
        ...
        for i in sample:
            ...
            mom = "WQ" + family + "-M.*"
            sample_mom = collection.find(
                {"sample_id": {"$regex": mom}, "analysised": "None"})
            for s in sample_mom:
                mom_id.append(s['sample_id'])
                mom_id = list(set(mom_id))
            preg = "WQ" + family + "-S.*"
            sample_preg = collection.find(
                {"sample_id": {"$regex": preg}, "analysised": "None"})
