...
class ReadsDistributionAgent(Agent):
    """
    ...
    def __init__(self, parent): 
        super(ReadsDistributionAgent, self).__init__(parent)
        options = [
    ...
    def check_options(self):
        ...
        if not self.option('bed').is_set:
            raise OptionError("请传入参考基因组结构注释bed格式文件") 
...
#        distribution = os.path.join(self.work_dir, "reads_distribution.txt")
 #       shutil.copyfile(distribution, "../ReadsDistribution/output/reads_distribution.txt")
    ...
    def run(self):
        ...
        运行 
        """
        super(ReadsDistributionTool, self).run()
