...
class StarAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            {"name":"ref_fa", "type":"infile", "format":"sequence.fasta"},#参考基因组文件 
            ...
            # 文件夹形式，输入一个fastq包含文件的文件夹{"name":"readFilesIN_dir", "type":"infile", "format":"sequence.fastq_dir"
        ]
...
class StarTool(Tool):
    ...
    def star_index1(self):
        ...
        cmd = "{}STAR --runMode genomeGenerate --genomeDir {} --genomeFastaFiles {}".format(self.star_path, self.genomeDir_path1, \
        self.option("ref_fa").prop["path"])  # self.work_dir/ref_star_index1 用于存放第一步建立的参考基因组索引的路径
        print cmd 
    ...
    def star_aln1(self):
        ...
        """
        cmd = "{}STAR --genomeDir {} --readFilesIn {} {}".format(self.star_path, self.genomeDir_path1, self.option("readFilesIN1").prop["path"], self.option("readFilesIN2").prop["path"])
        print cmd
    ...
    def star_index2(self,sj):
        ...
        """
        cmd = "{}STAR --runMode genomeGenerate --genomeDir {} --genomeFastaFiles {} --sjdbFileChrStartEnd {} --sjdbOverhang 75".format(self.star_path, self.genomeDir_path2, self.option("ref_fa").prop["path"], \
        sj)
        print cmd
    ...
    def run(self):
        ...
        if os.path.exists(os.path.join(self.work_dir, "SJ.out.tab")):
            SJ_out_tab = os.path.join(self.work_dir, "SJ.out.tab")
            self.star_index2(SJ_out_tab)
