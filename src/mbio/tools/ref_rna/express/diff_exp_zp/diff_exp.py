...
class DiffExpTool(Tool):
    ...
    def __init__(self, config):
        ...
        self._version = '1.0.1'
        self.edger = "/bioinfo/rna/trinityrnaseq-2.2.0/Analysis/DifferentialExpression/run_DE_analysis4.pl"
    ...
    def run_edger(self, dispersion=None):
        if self.option('edger_group').is_set:
            ...
            restart_edger_cmd = self.edger + " --matrix %s --method %3 --dispersion %s --output edger_result --min_rowSum_counts %s" % (self.option('count').prop['path'],  self.option('method'), dispersion, self.option('min_rowsum_counts'))
