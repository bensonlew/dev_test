...
from mbio.tools.ref_rna.diff_exp_zp.EDSeq2_stat import *
import os
import re
import itertools
import shutil
...
class DiffExpDeseq2Agent(Agent):
    ...
    def __init__(self, parent):
        super(DiffExpDeseq2Agent, self).__init__(parent)
        options = [
    ...
    def end(self):
        ...
        result_dir.add_regexp_rules([
            [r"_edgr_stat\.xls$", "xls", "DESeq2统计结果文件"]
        ])
        result_dir.add_relpath_rules(relpath)
        super(DiffExpDeseq2Agent, self).end()


class DiffExpDeseq2Tool(Tool):
    """
    表达量差异检测tool
    """
    def __init__(self, config):
        super(DiffExpDeseq2Tool, self).__init__(config)
        self._version = '1.0.1'
    ...
    def run(self):
        super(DiffExpDeseq2Tool, self).run()
        self.run_DESeq2()
        self.re_run_DESeq2()
