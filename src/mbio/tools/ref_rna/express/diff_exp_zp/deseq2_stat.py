...
def stat_deseq2(deseq2_result, countfile, fpkmfile, control, other, output, replicates=None, diff_ci=0.05, regulate=True):
    """对edgeR结果进行统计，获得两两分组（或样本）的edgeR统计文件以及差异基因列表文件"""
    with open(edseq2_result, 'rb') as e, open('%s/%s_vs_%s_edgr_stat.xls' % (output, control, other), 'wb') as w:
        eline = e.readline()
