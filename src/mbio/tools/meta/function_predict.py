...
class FunctionPredictAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option("otu_table.xls").is_set:
            raise OptionError("请传入otu_table.xls文件！")
        if not self.option("db") in ["kegg", 'cog', 'both']:
            raise OptionError("数据库必须为kegg或者cog,both！")
...
class FunctionPredictTool(Tool):
    ...
    def func_predict_analysis(self):
        ...
        cmd5 = '{}biom convert -i normalized_otus.biom -o normalized_otus.txt  --table-type "OTU table" --to-hdf5'.format(self.python_scripts)
        ...
        cmd8_ko = 'python {}predict_metagenomes.py -i normalized_otus.biom -o predictions_ko.biom -t ko'.format(self.picrust_path)
        ...
        cmd9_cog = '{}python {}predict_metagenomes.py -i normalized_otus.biom -o predictions_cog.biom -t cog'.format(self.python_scripts, self.picrust_path)
        ...
        cmd10 = '{}python {}categorize_by_function.py -i predictions_ko.biom -c KEGG_Pathways -l 1 -o predictions_ko.L1.biom'.format(self.python_scripts, self.picrust_path)
                ...
                if count == 0:
                    ...
                    for i in log_file:
                            if i.strip() == "Num OTUs:0":
                                self.set_error('16S功能预测需测序区域为16S rRNA，请检测测序引物序列。')
                else:
                    self.set_error('命令cmd{} ERROR，运行失败！'.format(count - 1))
