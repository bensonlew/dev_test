...
class RandomforestTool(Tool):
    def __init__(self, config):
        ...
        self.cmd_path = os.path.join(
            self.config.SOFTWARE_DIR, 'meta/scripts/RandomForest_perl.pl' )
    ...
    def run_RandomForest_perl(self):
        ...
        cmd += ' -i %s -o %s' % (real_otu_path, self.work_dir)
        if self.option('envtable').is_set:
