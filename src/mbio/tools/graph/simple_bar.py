...
class SimpleBarTool(Tool):
        ...
        if self.option("group_table").is_set:
            for i in range(len(self.option("group_table").prop['group_scheme'])):
                ...
                if self.option("calculation") == "none":  # 组内合并为none
                    ...
                    combined_txt = sample_dir + "/final_table.xls"
                    value_table = sample_dir + "/final_value.xls"
    ...
    def set_output(self):
        ...
        if self.option("calculation") == "none":
            ...
            for i in range(len(self.option("group_table").prop['group_scheme'])):
                os.link(self.work_dir + '/' + self.option("group_table").prop['group_scheme'][i] + '/final_value.xls', self.output_dir + '/' + self.option("group_table").prop['group_scheme'][i] + '_final_value.xls')
                os.link(self.work_dir + '/' + self.option("group_table").prop['group_scheme'][i] + '/final_table.xls', self.output_dir + '/' + self.option("group_table").prop['group_scheme'][i] + '_matrix_bar.xls')
    ...
    def run(self):
        ...
        if self.option('group_table').is_set:
            if self.option('method') == 'column':
                ...
                self.logger.info(self.option('input_table').prop['row_sample'])
                ...
                self.logger.info(self.option('group_table').prop['sample_name'])
                self.logger.info(self.option('input_table').prop['col_sample'])
                for i in self.option('group_table').prop['sample_name']:
