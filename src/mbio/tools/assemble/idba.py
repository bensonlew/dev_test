...
class IdbaAgent(Agent):
    ...
    def check_options(self):
        ...
        if not self.option('contig'):
            raise OptionError('必须传入输出文件名')
        if self.option('split_num') < 1:
            raise OptionError('reads拆分数目不能小于1')
        if self.option('mem') > 250 :
            raise OptionError('内存超出250G')
        return True
...
class IdbaTool(Tool):
    def __init__(self, config):
        ...
        self.idba_path = '/bioinfo/metaGenomic/MaxBin-2.2.1/auxiliary/idba-1.1.1/bin/'
    ...
    def run(self):
        ...
        if self.option('fastqs').is_set:
            ...
            os.link(self.work_dir + '/' + self.sample_name + '.pair.tmp.fa',self.work_dir + '/' + self.sample_name + '.fa')
        if self.option('split_num') > 1 :
            self.run_split()
    ...
    def run_fq2fa_cat(self):
        ...
        command = self.add_command("fq2fa_cmd3" , cmd3).run()
        self.wait(command)
        if command.return_code == 0:
            self.logger.info("run_fq2fa_cat done")
        else:
            self.set_error("run_fq2fa_cat error")
    ...
    def run_idba(self, number = 0):  #增加对样品分割后分别拼接的选择
        ...
        if not number:
            ...
            command = self.add_command("idba_cmd", cmd).run()
            self.wait(command)
            if command.return_code == 0:
                self.logger.info("idba组装完成")
            else:
                self.set_error("idba组装出错!")
        else:
            ...
            for i in range(1,number + 1):
                ...
                command = self.add_command("idba_cmd_{}".format(i), cmd).run()
                self.wait(command)
                if command.return_code == 0:
                    self.logger.info("idba组装完成")
                else:
