...
class BwaTool(Tool):
    ...
    def bwa_sampe(self, outfile, aln_l, aln_r, fastq_l, fastq_r):
        if self.option("head") is None:
            cmd = "{}bwa sampe -f {} {} {} {} {} {}".format(self.bwa_path, outfile, self.option("ref_fasta").prop["path"], aln_l, aln_r, fastq_l, fastq_r)
        else:
    ...
    def bwa_samse(self, outfile, aln_s, fastq_s):
        if self.option("head") is None:
            cmd = "{}bwa samse -f {} {} {} {}".format(self.bwa_path, outfile, self.option("ref_fasta").prop["path"], aln_s, fastq_s)
        else:
    ...
    def multi_sam(self):
        ...
        for sample in samples:
            if self.option("fq_type") in ["PE"]:
                ...
                # self.logger.info(fq_r, fq_l)
                sam_pe_cmd = self.bwa_sampe("{}.sam".format(sample), "{}_l.sai".format(sample), "{}_r.sai".format(sample), fq_l, fq_r)
                sam_commands.append(sam_pe_cmd)
            elif self.option("fq_type") in ["SE"]:
                fq_s = os.path.join(self.option("fastq_dir").prop["path"], samples[sample])
                sam_se_cmd = self.bwa_samse("{}.sam".format(sample), "{}_s.sai".format(sample), fq_s)
