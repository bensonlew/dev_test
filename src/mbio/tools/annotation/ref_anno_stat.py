...
class RefAnnoStatAgent(Agent):
    ...
    def __init__(self, parent):
        ...
        options = [
            ...
            {"name": "swissprot_xml", "type": "infile", "format": "align.blast.blast_xml"},  # 1
            {"name": "database", "type": "string", "default": "nr,go,cog,kegg,swissprot"},  # 2
    ...
    def check_options(self):
        ...
        for i in self.anno_database:
            ...
            if i == 'swissprot' and not self.option('swissprot_xml').is_set:  # 3
                raise OptionError('缺少swissprot注释的输入文件')  # 4
        ...
        with open(all_anno_stat, 'wb') as w:
            ...
            for i in self.database:
                if i == "cog":
                    ...
                    gene_cog_list = self.work_dir + '/cog_stat/' + 'gene_cog_list.xls'
                    anno_num['cog']['gene'] = len(self.list_num(gene_cog_list))
                    anno_num['cog']['tran'] = len(self.list_num(cog_list))
                    w.write('{}\t{}\t{}\t{}\t{}\n'.format(i, anno_num['cog']['tran'], anno_num['cog']['gene'], '%0.4g' % (anno_num['cog']['tran'] / anno_num['total']['tran']), '%0.4g' % (anno_num['cog']['gene'] / anno_num['total']['gene'])))
                if i == "go":
                    go_list = self.option("gos_list").prop["path"]
                    gene_go_list = self.option("gene_go_list").prop["path"]
                    anno_num['go']['gene'] = len(self.list_num(gene_go_list))
                    anno_num['go']['tran'] = len(self.list_num(go_list))
                    w.write('{}\t{}\t{}\t{}\t{}\n'.format(i, anno_num['go']['tran'], anno_num['go']['gene'], '%0.4g' % (anno_num['go']['tran'] / anno_num['total']['tran']), '%0.4g' % (anno_num['go']['gene'] / anno_num['total']['gene'])))
