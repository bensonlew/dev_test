...
class CardAnnotationModule(Module):
    def __init__(self, work_id):
        ...
        options = [
            ...
            {"name": "card_result_dir", "type": "outfile", "format": "meta_genomic.nr_dir"}  # 设置结果文件后面要用
        ]
    ...
    def run_split_fasta(self):
        ...
        self.split_fasta.on('end', self.card_align)          #########split发生end事件触发run.align
    ...
    def card_align(self):
        ...
        for f in os.listdir(self.split_fasta.output_dir):
            ...
            #align_diamond.on('end', self.set_output, 'card_align')
        ...
        if len(self.card_align_tools) > 1:
            ...
            #self.on_rely(self.card_align_tools, self.link_xml)
        else:
            self.card_align_tools[0].on('end', self.card_anno)
            #self.card_align_tools[0].on('end', self.link_xml)
    ...
    def card_anno(self):
        ...
        #xml_dir = self.card_align.output_dir.prop['path']
        self.card_anno_tool.set_options({
            "card_xml_dir": xml_dir
        })
        self.card_anno_tool.on('start', self.set_step, {'start': self.step.anno})
        self.card_anno_tool.on('end', self.set_step, {'end': self.step.anno})
        #self.card_anno_tool.on('end', self.set_output, "anno")
    ...
    def card_anno_stat(self):
        ...
        #self.card_anno_stat_tool.on('start', self.set_step, {'start': self.step.anno_stat})
        #self.card_anno_stat_tool.on('end', self.set_step, {'end': self.step.anno_stat})
