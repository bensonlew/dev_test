#!/mnt/ilustre/users/sanger/app/Perl/bin/perl
use strict;
use warnings;
use Getopt::Long;
my %opts;
...
my $usage = <<"USAGE";
       ...
       Version : $VERSION
       Contact : chenghai.yan\@majorbio.com
...
if(map !=\"none\"){
                ...
                sd\$$opts{g} <-as.factor(sd\$$opts{g} )
                legend <- as.matrix(unique(sd\$$opts{g})) 
}

set.seed(1)
if(map != \"none\"){
otu.rf <- randomForest(sd\$$opts{g} ~ .,otu,importance=T,proximity=T,ntree=$opts{ntree})
...
otu.mds\$points <- cmdscale(1-otu.rf\$proximity)
}
...
write.table(otu.mds\$points,mds_points,sep=\"\\t\",quote=F)
...
top_vimp <- paste(\"$opts{o}/\",basename,\"_top$opts{top}_vimp.xls\",sep=\"\")
imp <- importance(otu.rf)
if($opts{type} == 1){
	top <- imp[order(imp[,\"MeanDecreaseAccuracy\"],decreasing=T),][1:$opts{top},]
	write.table(t(otu)[rownames(top),],topx_vimp,sep=\"\\t\",quote=F)
	
}else if ($opts{type} == 2){
	top <- imp[order(imp[,\"MeanDecreaseGini\"],decreasing=T),][1:$opts{top},]
        write.table(t(otu)[rownames(top),],topx_vimp,sep=\"\\t\",quote=F)
}
