...
plot_KO <-function(xml_KO,bg_col,fg_col,height,err_x_location,err_y_location){
  ...
  i_max_fg<-length(fg_col)
  i_max_bg<-length(bg_col)
  if(i_max_fg==1){
    for (i in 1:i_max_fg) {
      plot_polygon(xml_KO,i=i,i_max=1,col=bg_col,height=height,err_x_location=err_x_location,err_y_location=err_y_location,magnify=1,border = fg_col)
    }
  }else{
    for (i in 1:i_max_bg) {
      plot_polygon(xml_KO,i=i,i_max=i_max_bg,col=bg_col,height=height,err_x_location=err_x_location,err_y_location=err_y_location,magnify=1,border = NA)
    }
    #i_max<-length(fg_col)
    for (i in 1:i_max_fg) {
      plot_segments(xml_KO,i=i,i_max=i_max_fg,col=NA,border=fg_col,height=height,err_x_location=err_x_location,err_y_location=err_y_location,magnify=1)
    }
  }
}
