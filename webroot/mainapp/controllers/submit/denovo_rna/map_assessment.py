...
class MapAssessment(object):
    ...
    def get_insert_data(self, analysis_type, client, express_info,  data, member_id):
        ...
        options = {'analysis_type': data.analysis_type, "express_id": data.express_id}
        ...
        if analysis_type == "saturation":
            rpkm_id = DenovoRnaMapping().add_rpkm_table(params=params, name=name, detail=False)
            ...
            coverage_id = DenovoRnaMapping().add_rpkm_table(params=params, name=name, detail=False)
            update_info = {str(coverage_id): "sg_denovo_coverage", 'database': self.db_name}
            update_info = json.dumps(update_info)
            options["update_info"] = update_info
            options.update(params)
        elif analysis_type == "duplication":
            duplication_id = DenovoRnaMapping().add_rpkm_table(params=params, name=name, detail=False)
            ...
            correlation_id = DenovoRnaMapping().add_rpkm_table(params=params, name=name, detail=False)
