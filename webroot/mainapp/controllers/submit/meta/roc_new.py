...
class RocNew(MetaController):
    def __init__(self):
        super(RocNew, self).__init__(instant=False)
    ...
    def POST(self):
        ...
        options = {
            ...
            "ran_for_analysis": data.ran_for_analysis,
            "otu_id": data.otu_id,
            "lefse_otu": data.lefse_otu,
            "lefse_group": data.lefse_group,
            "group_detail": data.group_detail,
            "second_group_detail": data.second_group_detail,
            "group_name": data.group_name,
            "two_ran_otu": data.two_ran_otu,
            "two_ran_group": data.two_ran_group,
            "level_id": data.level_id,
            "roc_calc_method": data.roc_calc_method,
            "roc_new_id": str(main_table_id),
    ...
    def check_options(self, data):
        ...
        for names in params_name:
            if not (hasattr(data, names)):
                success.append("缺少参数!")
        if data.strict and int(data.strict) not in [1, 0]:
            info = {"success": False, "info": "严格性比较策略不在范围内！"}
            return json.dumps(info)
        if data.lda_filter:
            if float(data.lda_filter) > 4.0 or float(data.lda_filter) < -4.0:
                success.append("LDA阈值不在范围内")
        if data.start_level and int(data.start_level) not in [1, 2, 3, 4, 5, 6, 7, 8, 9]:
            success.append('起始分类水平不在范围内')
        if data.end_level and int(data.end_level) not in [1, 2, 3, 4, 5, 6, 7, 8, 9]:
